...
static int GetKey(RedisModuleCtx *ctx, RedisModuleString *keyName, SKETCH **sketch, int mode)
{
    ...
    if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY)
    {
        ...
        ERROR("Tower_CM: key does not exist");
    }
...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY)
    {
        RedisModule_ReplyWithError(ctx, "Tower_CM: key already exists");
        goto final;
    }
...
int TowerCMModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    SKETCHType = RedisModule_CreateDataType(ctx, "TOWER__CM", TOWER_CM_ENC_VER, &tm);
    if (SKETCHType == NULL)
        return REDISMODULE_ERR;

    RMUtil_RegisterWriteDenyOOMCmd(ctx, "tower_cm.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "tower_cm.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "tower_cm.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "tower_cm.info", Info_Cmd);
