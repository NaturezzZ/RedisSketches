...
public:
    ...
    basic_sketch_reply()
    {
        cnt = 0;
        limit = 5;
        types = (basic_sketch_reply_type *)CALLOC(5, sizeof(basic_sketch_reply_type));
    ...
    void adjust()
    {
        ...
        limit *= 2;
        basic_sketch_reply_type *tmp_types = (basic_sketch_reply_type *)CALLOC(limit, sizeof(basic_sketch_reply_type));
        for (size_t i = 0; i < cnt; ++i)
            tmp_types[i] = types[i];
        FREE(types);
        types = tmp_types;
...
typedef basic_sketch_reply *(*basic_sketch_func)(void *o, const int &argc, const basic_sketch_string *argv);
...
public:
    ...
    static basic_sketch_string command_name(int index) { return basic_sketch_string(); }
...
template <class T>
static int GetKey(RedisModuleCtx *ctx, RedisModuleString *keyName, T **sketch, int mode)
{
...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_ModuleTypeSetValue(key, SKETCHType1, sketch) == REDISMODULE_ERR)
    {
        goto final;
    }
...
static int Command_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    static_assert(std::is_base_of<basic_sketch, T>::value, "type error!");
    ...
    basic_sketch_string *argv_new = convert_argv(argv + 2, argc - 2);
    basic_sketch_reply *result = T::command(func_index)(sketch, argc - 2, argv_new);
    result->reply(ctx);
    delete result;
    delete[] argv_new;
...
int add_command1(RedisModuleCtx *ctx)
{
    static_assert(std::is_base_of<basic_sketch, T>::value, "type error!");

    if (x <= T::command_num())
    {
        // fprintf(stderr, "%s\n", (T::class_name() + basic_sketch_string(".") + T::command_name(x)).c_str());
        switch (T::command_type(x))
        {
        case 0:
            RMUtil_RegisterWriteCmd(ctx, (T::class_name() + basic_sketch_string(".") + T::command_name(x)).c_str(), (Command_Cmd<T, x>));
            break;
        case 1:
            RMUtil_RegisterReadCmd(ctx, (T::class_name() + basic_sketch_string(".") + T::command_name(x)).c_str(), (Command_Cmd<T, x>));
            break;
        default:
            break;
        }
...
template <class T>
int Basic_Sketch_Module_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    static_assert(std::is_base_of<basic_sketch, T>::value, "type error!");
    ...
    RMUtil_RegisterWriteCmd(ctx, (T::class_name() + basic_sketch_string(".create")).c_str(), (Create_Cmd<T>));
