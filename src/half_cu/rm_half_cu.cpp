...
static int GetKey(RedisModuleCtx *ctx, RedisModuleString *keyName, SKETCH **sketch, int mode)
{
    ...
    if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY)
    {
        ...
        ERROR("HalfCU: key does not exist");
    }
...
int HalfCUModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    SKETCHType = RedisModule_CreateDataType(ctx, "CUSK-TYPE", CU_ENC_VER, &tm);
    if (SKETCHType == NULL)
        return REDISMODULE_ERR;

    RMUtil_RegisterWriteDenyOOMCmd(ctx, "half_cu.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "half_cu.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "half_cu.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "half_cu.info", Info_Cmd);
