...
static int GetKey(RedisModuleCtx *ctx, RedisModuleString *keyName, SKETCH **sketch, int mode)
{
    ...
    if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY)
    {
        ...
        ERROR("Tower_CU: key does not exist");
    }
...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY)
    {
        RedisModule_ReplyWithError(ctx, "Tower_CU: key already exists");
        goto final;
    }
...
static int Info_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RedisModule_ReplyWithLongLong(ctx, sketch->w[0]);
...
static void RdbSave(RedisModuleIO *io, void *obj)
{
    ...
    RedisModule_SaveStringBuffer(io, (const char *)sketch->w, sketch->d * sizeof(uint32_t));
    RedisModule_SaveStringBuffer(io, (const char *)sketch->A, sketch->w[0] * sketch->d * sizeof(uint32_t));
}

static void *RdbLoad(RedisModuleIO *io, int encver)
{
    if (encver > TOWER_CU_ENC_VER)
    {
        return NULL;
    }
    ...
    size_t size;
    sketch->w = (uint32_t *)RedisModule_LoadStringBuffer(io, &size);
    assert(size == sketch->d * sizeof(uint32_t));
    sketch->A = (uint32_t *)RedisModule_LoadStringBuffer(io, &size);
    assert(size == sketch->w[0] * sketch->d * sizeof(uint32_t));
...
{
    SKETCH *sketch = (SKETCH *)value;
    return sizeof(SKETCH) + sketch->w[0] * sketch->d * sizeof(uint32_t) + 2 * sketch->d * sizeof(uint32_t);
}

int TowerCUModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    SKETCHType = RedisModule_CreateDataType(ctx, "TOWER__CU", TOWER_CU_ENC_VER, &tm);
    if (SKETCHType == NULL)
        return REDISMODULE_ERR;

    RMUtil_RegisterWriteDenyOOMCmd(ctx, "tower_cu.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "tower_cu.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "tower_cu.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "tower_cu.info", Info_Cmd);
