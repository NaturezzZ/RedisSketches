...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY)
    {
        RedisModule_ReplyWithError(ctx, "CM: key already exists");
        goto final;
    }
...
int CMModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    SKETCHType = RedisModule_CreateDataType(ctx, "CMSK-TYPE", CM_ENC_VER, &tm);
    if (SKETCHType == NULL)
        return REDISMODULE_ERR;

    RMUtil_RegisterWriteDenyOOMCmd(ctx, "cm.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "cm.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "cm.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "cm.info", Info_Cmd);
