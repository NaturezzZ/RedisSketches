...
static int Create_Cmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (RedisModule_KeyType(key) != REDISMODULE_KEYTYPE_EMPTY)
    {
        RedisModule_ReplyWithError(ctx, "CU: key already exists");
        goto final;
    }
...
int CUModule_onLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "cu.create", Create_Cmd);
    RMUtil_RegisterWriteDenyOOMCmd(ctx, "cu.insert", Insert_Cmd);
    RMUtil_RegisterReadCmd(ctx, "cu.query", Query_Cmd);
    RMUtil_RegisterReadCmd(ctx, "cu.info", Info_Cmd);
